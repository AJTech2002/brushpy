...
6. Ensure all primitives can be created on Python side and remove the load from the C++ side and keep it as Engine only, no drawing logic or even Metal stuff there except the compositor, that is what will allow a library to be created quickly
	1. Create support for `Primitive` and `InputPrimitive`
7. Go back and remove all the primitives and their remaining code everywhere

8. Support quick video rendering support
...
6. Layer 
	...
	4. Storage as a separate texture for session storage
